      ...
      <div className="row text-center">
        <div className="col">
          ...
          <img
            ...
            alt="Cerrar"
            width={"25px"}
            className="navbar-message-close"
          />
        </div>
      </div>
      ...
      <div className="row mb-3 mt-3">
        <div className="col w-100">
          <input
            type="text"
            placeholder="Buscar..."
            className="navbar-messages-input w-100"
          />
        </div>
      </div>
      ...
      {chatsInfo.map((chat) => {
        return (
          <div className="row mb-3" key={chat.id}>
            ...
            <div
              className="col-md-10 my-auto ps-4 zencluPointer"
              onClick={() => {
                setIsChatOpen(true);
                setSelectedChat(chat);
              }}>
              {chat.numberMessages ? (
                <p className="navbar-messages-number">{chat.numberMessages}</p>
              ) : (
                <></>
              )}
                ...
                {chat.online ? (
                  <>
                    <div className="col p-0 position-relative">
                      ...
                      <span className="navbar-messages-online-name">
                        {chat.name}
                      </span>
                    </div>
                  </>
      ...
      <div className="row">
        <div className="col-md-2 ps-3 my-auto text-center">
          <img
            src={backIcon}
            alt="Retroceder"
            className="zencluPointer navbar-profile-image"
            width={"15 px"}
            onClick={() => {
              setIsChatOpen(false);
            }}
        ...
        <div className="col-md-8 ps-3 my-auto">
          ...
          <div className="row">
            <div className="col p-0 zencluMuted">
              Última conexión a las {selectedChat.lastConnection}
            </div>
          </div>
          <img
            src={closeIcon}
      ...
      {chatMessages.map((message) => {
        return (
          <div className="row ps-3 pe-3" key={message.id}>
            ...
            <div
              className={`col-md-8 navbar-message-message 
            ${
              message.notLast
                ? message.isMine
                  ? "sendedMessageNotLast"
                  : "recivedMessageNotLast"
                : message.isMine
                ? "sendedMessage"
                : "recivedMessage"
            }`}>
